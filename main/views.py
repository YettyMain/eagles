...
def add_to_cart(request):
    if request.method == "POST":
        ...
        if cart:
            basket = Cart.objects.filter(user__username = request.user.username, product=product.id, quantity=quantity, price=product.price, paid=False).first()
            if basket:
                basket.quantity += quantity
                basket.amount = basket.quantity * product.price 
                basket.save()
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_eae18ef6d41db9494dd428cb76d66d252dff23dc' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://localhost:8000/thankyou' #thank you page
        ref = str(uuid.uuid4()) #refrence id required by paystack as an additional reference number
        order_no = profile.id
